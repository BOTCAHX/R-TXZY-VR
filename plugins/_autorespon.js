    ...
    try {
        if (m.mentionedJid.includes(this.user.jid) && m.isGroup) {
            await this.send2Button(m.chat,
                isBanned ? 'Fachri Botz tidak aktif' : banned ? 'kamu dibanned' : 'Fachri Botz disini',
                '© Fachri',
...
╠═〘 Fachri Botz 〙 ═`.trim(), '© Fachri Botz', 'Gopay', '#viadana', 'Pulsa', '#viapulsa', m)
    ...
    if (new Date() * 1 - setting.status > 1000) {
        ...
        await this.setStatus(`🤖Iam Fachri Botz | ⏳Runtime: ${uptime} | 🖥️Mode: ${global.opts['self'] ? 'Private' : setting.groupOnly ? 'Hanya Grup' : 'Publik'}`).catch(_ => _)
